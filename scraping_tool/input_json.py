...
def input_table(mails):
    ...
    for mail in mails:
        ...
        try:            
            # INSERT
            data = cl.OrderedDict()
            data['sender'] = str(mail.sender).split("<")[-1].split('>')[0]
            data['sender_email_address'] = mail.senderEmailAddress
            data['received_time'] = received_time.isoformat()
    ...
    fw = open('mail_data_set.json', 'w')
    json.dump(data_set, fw, indent=4)
...
def scrape():
    ...
    # メールアドレスごとに処理
    address = outlook.Folders[0]

    # 受信フォルダを選択
    inbox = list(filter(choose_inbox, address.Folders))[0]
    
    # zoomアドレスを持つメールを集計
    zoom_mails = list(filter(contains_zoom, inbox.Items))
